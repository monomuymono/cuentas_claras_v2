...
import { createPortal } from 'react-dom'; // <-- AÑADIDO: Importación para Portales
...
// Componente para el modal de resumen, AHORA USANDO UN PORTAL
const SummaryModal = ({ isOpen, onClose, summaryData }) => {
  if (!isOpen) return null;

  // Este es el JSX que se renderizará en el portal
  const modalContent = (
  ...
  // Usamos createPortal para renderizar el modal en el div #modal-root de tu index.html
  return createPortal(modalContent, document.getElementById('modal-root'));
};

// Nuevo componente de estilos de impresión, simplificado para funcionar con el portal
const PrintStyles = () => (
    <style>
    {`
      @media print {
        /* Oculta el contenedor principal de la aplicación React */
        #root {
          display: none;
        }
        /* El #modal-root y su contenido serán visibles por defecto */
        /* Nos aseguramos de que el contenido del resumen ocupe toda la página */
        #summary-to-print {
          position: absolute;
          left: 0;
          top: 0;
...
const App = () => {
  ...
  // ... (Toda la lógica de guardado, carga y efectos permanece idéntica a la versión anterior y es correcta)
